...
> **anyviz** 是一个面向 AI 时代的数据可视化规范与 Claude Skill 库。它为 AI 助手赋予专业数据
> 设计师般的判断力：智能图表选择、严谨调校的美学系统、多技术栈渲染，以及同一画布下多图表的
> 设计一致性。
...
大模型早已能写出绘图代码，缺的是**品味**与**一致性**——那些把"默认样式的图"与"出版级作品"
区分开的成百上千个细小决策。anyviz 把这些决策沉淀为一套可复用的规范：选哪种图、用什么色板、
字号如何分级、标签放在哪里，以及十张图如何保持视觉连贯。
...
将本仓库克隆到本地，用自然语言描述需求即可。Claude 会加载 `SKILL.md` 并自动遵循 anyviz 流水线。
