...
7. **图例与维度标签避让**：图例放置在画布底部时，必须避开底部维度标签的 y 坐标区域。具体规则：
   - 底部维度标签 y = 圆心 y + R + 22px（R 为雷达图半径）
   - 图例 y 起始 ≥ 底部维度标签 y + 22px
   - 若图例项 ≥ 4，使用 2×N 网格布局（见 `guides/legend-layout.md` 规则 3），行间距 22px
   - 图例水平居中于画布（见 `guides/legend-layout.md` 规则 4）
8. **多实体线型区分（academic 主题）**：使用 academic 灰度主题时，实体间仅靠灰度差异不够明显，必须叠加线型差异：第 1 实体实线、第 2 长虚线(5,3)、第 3 短虚线(3,2)、第 4 点线(1,2)。图例标记线段必须使用相同 dasharray
